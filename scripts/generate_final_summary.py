...
def build(out_root: Path) -> str:
    ...
    L.append("")
    L.append("- 修正后 GARCH 族 1% 违例率 1.4-1.5%，**高于**名义 1%（即 mildly under-conservative：VaR 不够负、违例偏多，Kupiec p=0.01-0.03 拒绝）；HS 5%/10% 频率最接近目标但独立性检验 p<0.001（违例强聚集）。")
    L.append("- 条件充分性（逐模型）：违例独立性在 1%/5% 均未被明显拒绝；但条件覆盖并非普遍通过 —— 仅 GJR-t 的 1% CC 在 5% 水平不拒绝（p=0.089），GARCH-t 1% CC p=0.017 拒绝、两模型 5% CC 均拒绝（p=0.007/0.008）。正确频率 ≠ 正确条件 VaR 动态。")
    L.append("")
    ...
    L.append("")
    L.append("- GJR-t（M4）在 5%/10% tail 显著优于 GARCH-t（Holm p=0.006/0.011，bootstrap 一致），1% tail 为方向性证据（Holm p=0.071 不显著）—— 非对称波动率设定带来预测损失改进，与 leverage/asymmetry 效应 consistent（本实验不做参数级 leverage 识别）。")
    ...
    L.append("| H3a RV 增量 | supported（主要在 5%/10% tail） | F0→F1 pinball 下降（线性 -4.4%） |")
    L.append("| H3b BV 条件增量 | unsupported | F1→F2 ≈ 0 |")
    L.append("| H4 MLP 绝对增量 | unsupported | 三 tail pinball 均不优于 GARCH 族；1%/10% Holm 显著更差 |")
    L.append("| H5 非线性映射价值 | unsupported / inconclusive | 10% tail 显著更差（Holm p=0.0015）；1%/5% 无清晰优势；joint loss+non-crossing 使其非纯 mapping control |")
    L.append("| H6 tail 依赖的相对表现 | supported | 校准与 DM 差异随 tail 明显变化（GJR 5/10% 显著、1% 方向性） |")
    L.append("| H7 jump/downside block 极端 tail 增量 | not identified | F3 为联合 block 且总体无稳定增量；无 component identification |")
    L.append("")
    ...
    L.append("- NN 计算成本约为经典模型 10 倍且无绝对增量；GRU 1% tail seed sensitivity（std 0.36%）。")
